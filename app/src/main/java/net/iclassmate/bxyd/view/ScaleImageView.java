...
public class ScaleImageView extends ImageView {
    ...
    private boolean canRotate = true; // 是否�?启旋转图片功�?
    private boolean canDoubleClick = true; // 是否�?启双击缩放功�?
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            break;
        // 副点按下 ：如果间距大�?10，记录此时matrix，以及两点的坐标、图片状�?
        case MotionEvent.ACTION_POINTER_DOWN:
            if (event.getActionIndex() > 1) // 超过两个点返�?
                break;
            dist = spacing(event.getX(0), event.getY(0), event.getX(1),
                    event.getY(1));
            if (dist > 10f) { // 如果连续两点距离大于10，则判定为多点模�?
                savedMatrix.set(matrix);
