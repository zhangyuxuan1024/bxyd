...
public class DateUtil { 
    ...
    public static String[] WEEKS_NAME = {"日", "一", "二", "三", "四", "五", "六"}; 
    ...
    public static int[] getDay(int year, int month, int day, int pervious) { 
        int[] time = new int[3]; 
        Calendar c = Calendar.getInstance(); 
        c.set(Calendar.YEAR, year); 
        c.set(Calendar.MONTH, month - 1);
        c.set(Calendar.DAY_OF_MONTH, day); 
        c.add(Calendar.DATE, pervious); 
...
//        }
    ...
    public static Date getDateFromString(int year, int month, int day) { 
        String dateString = year + "-" + (month > 9 ? month : (0 + month)) + "-" + day; 
        Date date = null; 
        try { 
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); 
            date = sdf.parse(dateString); 
