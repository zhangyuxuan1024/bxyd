...
public class CalendarMonthCard extends View { 
    ...
    private Paint mTextPaint2; // 绘制文本今天的画笔
        ...
        for (int j = 0; j < TOTAL_ROW; j++) { 
            ...
            for (int i = 0; i < TOTAL_COL; i++) { 
                ...
                if (position >= firstDayWeek 
                    ...
                    day++;
//                    Log.i("TAG", "position:"+position+",firstDayWeek:"+firstDayWeek+",currentMonthDays:"+currentMonthDays+",day:"+day);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mTextPaint2.setTextSize(mCellSpace / 4);
    } 
...
//            Log.i("hi","大于,todayDate:"+todayDate+",customDate"+date+",DateUtil.getCurrentMonthDay():"+DateUtil.getCurrentMonthDay());
            boolean isShow = true;
            switch (state) { 
            case TODAY: // 今天 
            ...
            if(isShow){
                ...
                if(j == 1 && date.day == 1){
                    ...
                    float startX= ConstantCalendar.dip2px(getContext(), 20);
...
//                    canvas.drawLine(mCellSpace*i, mCellSpace, mViewWidth, mCellSpace, mTextPaint);
                    canvas.drawLine(0, mCellSpace, mViewWidth, mCellSpace, mTextPaint);
                }
                mTextPaint.setColor(Color.parseColor(ConstantCalendar.COLOR_LINE_DEFAULT));
...
//                }
        ...
        public void drawContentToday(String content, Canvas canvas) {
            mTextPaint2.setColor(Color.parseColor(ConstantCalendar.COLOR_TODAY));
            canvas.drawText(content, (float) ((i + 0.5) * mCellSpace - mTextPaint.measureText(content) / 2 + 8),
                    (float) ((j + 0.77) * mCellSpace - mTextPaint.measureText(content, 0, 1) / 2 + 60), mTextPaint2);
        }
