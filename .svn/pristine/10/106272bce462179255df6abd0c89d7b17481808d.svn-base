 ...
 */
public class GroupSpaceListAdapter extends BaseAdapter {
    ...
    public GroupSpaceListAdapter(Context mContext, Group group) {
...
//        String url = lruCache.get(sessionId);
        ...
        if(url != null && !TextUtils.isEmpty(url)){
            if(sessionType == 3){
                Picasso.with(mContext).load(url).resize((int) mContext.getResources().getDimension(R.dimen.view_43),
                        ...
                        .placeholder(R.mipmap.ic_qunzu_guanzhu).error(R.mipmap.ic_qunzu_guanzhu).config(Bitmap.Config.RGB_565).into(holder.imageIcon);
            }else if(sessionType == 4){
                Picasso.with(mContext).load(url).placeholder(R.mipmap.ic_jigou_guanzhu).error(R.mipmap.ic_jigou_guanzhu).config(Bitmap.Config.RGB_565).into(holder.imageIcon);
            }
        }
        return convertView;
    }
    ...
    private void setImage(String url, int sessionType, ImageView imageIcon) {
        if (sessionType == 2) {
            ...
            if (url == null || url.equals("")) {
                imageIcon.setImageResource(R.mipmap.ic_qunzu_guanzhu);
            } else {
                Picasso.with(mContext).load(url).resize((int) mContext.getResources().getDimension(R.dimen.view_43),
                        (int) mContext.getResources().getDimension(R.dimen.view_43))
                        .placeholder(R.mipmap.ic_qunzu_guanzhu).error(R.mipmap.ic_qunzu_guanzhu).config(Bitmap.Config.RGB_565).into(imageIcon);
            }
        } else if (sessionType == 4) {
            if (url == null || url.equals("")) {
                imageIcon.setImageResource(R.mipmap.ic_jigou_guanzhu);
            } else {
                Picasso.with(mContext).load(url).placeholder(R.mipmap.ic_jigou_guanzhu).error(R.mipmap.ic_jigou_guanzhu).config(Bitmap.Config.RGB_565).into(imageIcon);
            }
        }
    }
