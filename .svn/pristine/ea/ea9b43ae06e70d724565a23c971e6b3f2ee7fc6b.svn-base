...
//			onStateChanged(ILoadingLayout.State.RESET, true);
            ...
            if (NetWorkUtils.isNetworkAvailable(UIUtils.getContext())) {
                mPullDownState = ILoadingLayout.State.SUCCEED;
                onStateChanged(ILoadingLayout.State.SUCCEED, true);
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        setInterceptTouchEventEnabled(true);
                        mHeaderLayout.setState(ILoadingLayout.State.SUCCEED);
                    }
                }, getSmoothScrollDuration());
            } else {
                mPullDownState = ILoadingLayout.State.FAIL;
                onStateChanged(ILoadingLayout.State.FAIL, true);

                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        setInterceptTouchEventEnabled(true);
                        mHeaderLayout.setState(ILoadingLayout.State.FAIL);
                    }
                }, getSmoothScrollDuration());
            }
